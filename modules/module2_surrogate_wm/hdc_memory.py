...
class HDCMemory:
    ...
    def __init__(self, config: dict, views: Dict[str, torch.Tensor]):
        ...
        self.dim = self.config["task"]["perceiver_tokens"]        
    ...
    def cleanup(self):
        """Stochastic cognitive dropout — Zero-Allocation View Safe"""
        if self.node_count == 0:
            return
            
        # Генерируем маску строго под количество активных узлов
        decay = torch.rand(self.node_count, device=self.device) < 0.01
        
        # Ограничиваем область centrality с помощью .narrow() до активных узлов
        # и применяем маску in-place без изменения общих размеров буфера
        self.centrality.narrow(0, 0, self.node_count).masked_fill_(decay, 0)
